...
## Script that reads alignment 1 (main), alignment 2 (secondary), 
## a file with the list of taxa to be extracted from alignment 2, 
## alignment 3, and a file for 3.
## Then, it attaches 
...
## <path_to_aln3> <list2_taxa_to_add>
...
open (INFILE4, "<$ARGV[3]") or die "Cannot open $ARGV[3] file: $!";
open (INFILE5, "<$ARGV[4]") or die "Cannot open $ARGV[4] file: $!";
...
# Parse third alignment 
my $name_taxa2 = "";
my $match_taxa2 = "";
foreach my $taxa (@add_fasta2){
	chomp( $taxa );
	$name_taxa2 = $taxa;
	foreach my $seq (@aln3){
		chomp( $seq );
		my @split_aln2 = split( /   /, $seq );
		$match_taxa2 = $split_aln2[0];
		chomp( $match_taxa2 );
		if( $name_taxa2 =~ /\Q$match_taxa2\E/ ){
