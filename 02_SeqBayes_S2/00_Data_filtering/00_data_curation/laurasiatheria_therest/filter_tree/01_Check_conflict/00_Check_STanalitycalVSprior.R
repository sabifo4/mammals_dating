...
#
# Node 660:  ST ( 1.6419, 0.4248, 12.6518, 1714.5649 )
# Node 661:  ST ( 0.6940, 0.0062, 0.4310, 4.9531 )
# Node 662:  ST ( 0.6780, 0.0056, 0.3705, 4.4640 )
# Node 664:  ST ( 0.6714, 0.0054, 0.3549, 4.3287 )
# Node 665:  ST ( 0.5383, 0.0271, -2.1152, 70.3753 )
# Node 666:  ST ( 0.4672, 0.0312, -1.9657, 117.0119 )
# Node 667:  ST ( 0.3971, 0.0313, -1.4152, 427.3910 )
# Node 917:  ST ( 0.1482, 0.0212, 0.4170, 367.2184 )
# Node 955:  ST ( 0.0699, 0.0152, 1.1198, 331.4972 )
# Node 969:  ST ( 0.6600, 0.0052, 0.3409, 4.2070 )
# Node 1000:  ST ( 0.5771, 0.0070, -0.6339, 7.5086 )
# Node 1005:  ST ( 0.5956, 0.0170, -1.2426, 19.8564 )
# Node 1008:  ST ( 0.5964, 0.0155, -1.2391, 13.5715 )
...
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.998805 * t )*'' ) ) )
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.001635 * t )*'' ) ) )
...
summary( lm.qup )
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 0.9993' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.031822 * t )*'' ) ) )
...
#
# A) E.g., 6.7% (mean deviation) of 15.471773 (My, x.mean) -->  1.036609
#    Let's subtract that deviation from x_i after dividing into 100
#       0.1482 - (0.01036609) = 0.1378339
#===================================================================================================#
...
#
# A) E.g., 1.2% (mean deviation Fe-Pa) of 0.02016945 (sd Felis-Panthera) --> 0.0002420334
#    Let's subtract that deviation from w_i
#       0.0212 - (0.0002420334) = 0.02095797
#
dub.pos <- c( 8, 9 )
...
text( x = 100, y = 180,
      labels = expression( atop( ''*italic( R^2 )*' =  1 ' ) ) )
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.99643 * t )*'' ) ) )
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1 ' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.001197 * t )*'' ) ) )
...
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.024872 * t )*'' ) ) )
...
#
# A) E.g., 1.7% (mean deviation) of 7.898192 (My, x.mean.2) --> 0.1342693
#    Let's subtract that deviation from x_i after dividing into 100
#       0.06310756 - (0.001342693) = 0.06176487
#===================================================================================================#
# Values to add to ST for next tweaking (scale ~ sd)
#   100                            ---> sd_est_divt.post72sp[ind.post.72sp[dub.pos.2[1]]]
#   sum.stats.ord.2[dub.pos.2[1],5]  ---> add.to.STwi = ?
#
# A) E.g., 27% (mean deviation) of 0.01221634 (sd) --> 0.003298412
#    Let's subtract that deviation from w_i (ST.calibs.tweak)
#       0.01669039 - (0.003298412) = 0.01339198
dub.pos.2              <- 9
...
text( x = 100, y = 180,
      labels = expression( atop( ''*italic( R^2 )*' =  1' ) ) )
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.9982407 * t )*'' ) ) )
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.002103 * t )*'' ) ) )
...
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 0.9993' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.031747 * t )*'' ) ) )
