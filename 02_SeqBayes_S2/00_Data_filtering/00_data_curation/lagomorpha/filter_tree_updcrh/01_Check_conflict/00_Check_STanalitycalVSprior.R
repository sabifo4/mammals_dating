...
SN.calibs.nodes.names <- c( "LAGOMORPHA" )
...
ST.SN.dists <- define_ST( log_ST = "00_Prior_onlyST/log.mammals.mcmc1.txt",
                          ...
                          SN = TRUE,
                          SN.calibs.nodes.names =  SN.calibs.nodes.names )
attach( ST.SN.dists )
save( ST.SN.calib.nodes, file = "outRdata/ST.SN.calib.nodes.RData" )
...
ind.post.cal <- ind.post.72sp <- rep( 0, length( ST.SN.calib.nodes ) )
names( ind.post.72sp ) <- names( ind.post.cal ) <- names( ST.SN.calib.nodes ) 

for( i in 1:length( ind.post.cal ) ){
  ind.post.cal[i]  <- which( colnames( onlyST$divt ) == names( ST.SN.calibs )[i] )
...
plot_func( post.cal = onlyST$divt, ind.post.cal = ind.post.cal,
           ST.cal = ST.SN.calibs, ST.cal.nodes = names( ST.SN.calibs ),
           post.72sp = divtimes.post72sp, ind.post.72sp = ind.post.72sp,
           names.calibs = c(ST.calib.nodes.names, SN.calibs.nodes.names),
...
plot_func_mcmc( post.cal.1 = onlyST$divt1, post.cal.2 = onlyST$divt2,
                ind.post.cal = ind.post.cal, ST.cal = ST.SN.calibs,
                ST.cal.nodes = names( ST.SN.calibs ),
                post.72sp = divtimes.post72sp, ind.post.72sp = ind.post.72sp,
...
plot_func( post.cal = STnSB$divt, ind.post.cal = ind.post.cal,
           ...
           names.calibs = ST.calib.nodes.names,
           p.nrow = p.nrow, p.ncol = p.ncol, out = TRUE, legend = TRUE,
           outname = paste( "01_SBnSTnSN_", subtree, sep = "" ) )
...
plot_func_mcmc( post.cal.1 = STnSB$divt1, post.cal.2 = STnSB$divt2,
                ...
                p.nrow = p.nrow, p.ncol = p.ncol, out = TRUE,
                legend = TRUE,
                outname = paste( "01_SBnSTnSN_", subtree, "_MCMCruns", sep ="" ) )
...
pdf( file = paste( "01_SBnSTnSN_", subtree, "_meanquant.pdf", sep = "" ),
     paper = "a4", width = 1024, height = 768 )
curr_plot <- dev.cur()
png( filename = paste( "01_SBnSTnSN_", subtree, "_meanquant.png", sep = "" ),
...
col.mean <- rep( "black", length( ST.SN.calib.nodes ) )
cex.lab <- rep( 0.3, length( ST.SN.calib.nodes ) )
# col.mean[dub.pos] <- "red"
# cex.lab[dub.pos] <- 0.7
plot( x = x.mean, y = y.mean, pch = 20, col = col.mean,
      xlab = expression( 'Posterior mean time '*italic(t)*' (Ma)|72sp - data1' ),
      ylab = expression( 'Posterior mean time '*italic(t)*' (Ma)|88sp - lagomorpha_data2' ),
      xlim = c( min( x.mean )-5, max( x.mean )+ 90 ) )
coords.m <-  list( x = x.mean + 60, y = y.mean )
text( coords.m$x, coords.m$y, labels = c(ST.calib.nodes.names, SN.calibs.nodes.names), cex= cex.lab, col = col.mean )
...
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.993750 * t )*'' ) ) )
...
plot( x = x.qlow, y = y.qlow, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|88sp - lagomorpha_data2' ),
      xlim = c( min( x.qlow )-5, max( x.qlow )+ 90 ) )
coords.ql <-  list( x = x.qlow + 60, y = y.qlow )
text( coords.ql$x, coords.ql$y, labels = c(ST.calib.nodes.names, SN.calibs.nodes.names), cex= cex.lab, col = col.mean )
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1 ' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  0.998545 * t )*'' ) ) )
...
plot( x = x.qup, y = y.qup, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|88sp - lagomorpha_data2' ),
      xlim = c( min( x.qup )-5, max( x.qup )+ 100 ) )
coords.qu <-  list( x = x.qup + 70, y = y.qup )
text( coords.qu$x, coords.qu$y, labels = c(ST.calib.nodes.names, SN.calibs.nodes.names), cex= cex.lab, col = col.mean )
...
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.036889 * t )*'' ) ) )
...
rownames( sum.stats ) <- paste( ST.SN.calib.nodes, names( ST.SN.calib.nodes), sep = "-")
sum.stats.ord <- round( sum.stats*100, digits = 1 )

write.table( x = sum.stats.ord, file = paste( "01_SBnSTnSN_", subtree, "_sumstats.tsv", sep = "" ),
             sep = "\t", quote = FALSE )
