...
wd2 <- gsub( pattern = "00_data_curation..*", replacement = "",
             x = wd )
# Set name 
tname <- "Rodentia_therest"

#=================#
# 2. SCAN TREES   #
#=================#
# Get trees 
tree_original <- readLines( con = "rodentia_therestII_rooted_calibnames.tree" )
tree <- tree_original

# Get sp names 
spnames <- ape::read.tree( "rodentia_therestII_rooted_calibnames.tree" )$tip.label

#======================================#
#    3. GET FILE WITH CALIBRATIONS 
#======================================#
# Read tree and rename cols
calibrations <- read.table( file = "Calibrations_rodtherest.txt",
...
num_sp <- length( ape::read.tree( "rodentia_therestII_rooted_calibnames.tree" )$tip.label )
