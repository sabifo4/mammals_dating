...
#
# Node 692:  ST ( 1.6419, 0.4248, 12.6518, 1714.5649 ) --> MAMMALIA 
# Node 693:  ST ( 0.1744, 0.0351, 3.1168, 381.9704 )   --> MURIDAE 
# Node 698:  ST ( 0.0889, 0.0241, 3.7938, 547.4454 )   --> MURINAE
# Node 752:  ST ( 0.0432, 0.0128, 3.5750, 94.2223 )    --> M.pahari-rest
# Node 770:  ST ( 0.0222, 0.0068, 3.4256, 86.2455 )    --> M.caroli-rest
# Node 771:  ST ( 0.0104, 0.0033, 3.3802, 83.5293 )    --> M.spretus-M.musculus
# Node 1379:  ST ( 0.1366, 0.0302, 3.4638, 240.3174 )  --> CRICETIDAE
...
onlyST <- load_data( mcmc1 = "01_Prior_SBandST/mcmc1/mcmc.txt", mcmc2 = "01_Prior_SBandST/mcmc2/mcmc.txt" )
...
#============================================================================#
# Load files and generate objects with sum stats using in-house function `load_data`
STnSB.2 <- load_data( mcmc1 = "02_Prior_SBandST_tw1/mcmc1/mcmc.txt", mcmc2 = "02_Prior_SBandST_tw1/mcmc2/mcmc.txt" )
...
plot_func( post.cal = STnSB.2$divt, ind.post.cal = ind.post.cal,
           ...
           names.calibs = ST.calib.nodes.names,
           p.nrow = p.nrow, p.ncol = p.ncol, out = TRUE, legend = TRUE,
           outname = paste( "02_tw1_", subtree, sep = "" ) )
...
plot_func_mcmc( post.cal.1 = STnSB.2$divt1, post.cal.2 = STnSB.2$divt2,
                ...
                p.nrow = p.nrow, p.ncol = p.ncol, out = TRUE,
                legend = TRUE,
                outname = paste( "02_tw1_", subtree, "_MCMCruns", sep ="" ) )
...
plot( x = x.mean, y = y.mean, pch = 20, col = col.mean,
      ...
      ylab = expression( 'Posterior mean time '*italic(t)*' (Ma)|691sp - rodentia_subt2_data2' ),
...
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.994230 * t )*'' ) ) )
...
plot( x = x.qlow, y = y.qlow, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|691sp - rodentia_subt2_data2' ),
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  0.9999301 * t )*'' ) ) )
...
plot( x = x.qup, y = y.qup, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|691sp - rodentia_subt2_data2' ),
...
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.041778 * t )*'' ) ) )
...
#
# A) E.g., 2.6% (mean deviation) of 10.76269  (My, x.mean) -->  0.2798299 
#    Let's subtract that deviation from x_i (ST.calibs[[dub.pos[1]]])after dividing into 100
#       0.0889 - 0.002798299 = 0.0861017
#===================================================================================================#
...
#
# A) E.g., 17.9% (mean deviation) of 0.01517994    (sd ) --> 0.002717209
#    Let's subtract that deviation from w_i
#       0.0241 - 0.002717209 = 0.02138279
#
dub.pos         <- 3
...
text( x = 100, y = 180,
      labels = expression( atop( ''*italic( R^2 )*' =  1' ) ) )
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.994749 * t )*'' ) ) )
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  0.999583 * t )*'' ) ) )
...
plot( x = x.qup.2, y = y.qup.2, col = col.mean, pch = 20,
      xlab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' ' ),
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|691sp - rodentia_subt2_data2' ),
...
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.04411 * t )*'' ) ) )
