...
#
# Node 635:  ST ( 1.6419, 0.4248, 12.6518, 1714.5649 )
# Node 636:  ST ( 0.5964, 0.0155, -1.2391, 13.5715 )

# 0. Set dir name and get names of the calibrated nodes -- manually done
#    after checking excel file
subtree <- "L.chiroptera_subt2"
...
plot( x = x.mean, y = y.mean, pch = 20, col = col.mean,
      ...
      ylab = expression( 'Posterior mean time '*italic(t)*' (Ma)|634sp - chiroptera_subt2_data2' ),
...
text( x = 100, y = 177,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.994767 * t )*'' ) ) )
...
plot( x = x.qlow, y = y.qlow, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|634sp - chiroptera_subt2_data2' ),
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1 ' ) ) )
text( x = 100, y = 148,
      labels = expression( atop( ''*italic( w )*' = '*italic(  0.9992943 * t )*'' ) ) )
...
plot( x = x.qup, y = y.qup, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|634sp - chiroptera_subt2_data2' ),
...
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.040363 * t )*'' ) ) )
