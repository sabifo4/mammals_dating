...
wd2 <- gsub( pattern = "00_data_curation..*", replacement = "",
             x = wd )
# Set name 
tname <- "Laurasiatheria_chiroptera"

#=================#
# 2. SCAN TREES   #
#=================#
# Get trees 
tree_original <- readLines( con = "laurasiatheria_chiropteraII_calibnames.tree" )
tree <- tree_original

# Get sp names 
spnames <- ape::read.tree( "laurasiatheria_chiropteraII_calibnames.tree" )$tip.label

#======================================#
#    3. GET FILE WITH CALIBRATIONS 
#======================================#
# Read tree and rename cols
calibrations <- read.table( file = "Calibrations_LaurChiroptera.txt",
...
num_sp <- length( ape::read.tree( "laurasiatheria_chiropteraII_calibnames.tree" )$tip.label )
