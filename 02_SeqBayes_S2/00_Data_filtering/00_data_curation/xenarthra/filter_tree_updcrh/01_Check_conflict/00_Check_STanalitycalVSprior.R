...
# Node  34:  ST ( 1.6419, 0.4248, 12.6518, 1714.5649) --> MAMMALIA
# Node  35:  ST ( 0.6281, 0.0194, -0.3891, 4.8802 )   --> XENARTHRA
#
...
plot( x = x.mean, y = y.mean, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Posterior mean time '*italic(t)*' (Ma)|33sp - xenarthra_data2' ),
...
text( x = 100, y = 177,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.994788  * t )*'' ) ) )
...
plot( x = x.qlow, y = y.qlow, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|33sp - xenarthra_data2' ), 
...
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 1' ) ) )
text( x = 100, y = 148,
      labels = expression( atop( ''*italic( w )*' = '*italic(  0.999772 * t )*'' ) ) )
...
plot( x = x.qup, y = y.qup, col = col.mean, pch = 20,
      ...
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|33sp - xenarthra_data2' ),
      xlim = c( min( x.qup )-5, max( x.qup )+ 80 ) )
coords.qu <-  list( x = x.qup + 50, y = y.qup )
...
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.04193 * t )*'' ) ) )
