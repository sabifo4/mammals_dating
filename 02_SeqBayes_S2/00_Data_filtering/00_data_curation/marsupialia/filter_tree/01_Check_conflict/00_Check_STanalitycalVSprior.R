...
#
# Node 308:  ST ( 1.6419, 0.4248, 12.6518, 1714.5649 )
# Node 310:  ST ( 0.5623, 0.0804, 0.0351, 299.8367 )
# Node 311:  ST ( 0.4592, 0.0683, 0.0249, 623.4335 )
...
subtree <- "Marsupialia"
ST.calib.nodes.names <- c( "MAMMALIA", "DIDELPHIS-MACROPUS",
                           "EOMETATHERIA"
)
...
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 1.0037 * t )*'' ) ) )
...
summary( lm.qlow )
text( x = 100, y = 150,
      labels = expression( atop( ''*italic( R^2 )*' = 0.9976 ' ) ) )
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.01355 * t )*'' ) ) )
...
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.044228 * t )*'' ) ) )
...
#=================#
# Extract mean/quant for results when both SB and ST are added
pdf( file = paste( "02_SBnST_max0.3_pU0.025_", subtree, "_meanquant.pdf", sep = "" ),
     paper = "a4", width = 1024, height = 768 )
curr_plot <- dev.cur()
png( filename = paste( "02_SBnST_max0.3_pU0.025_", subtree, "_meanquant.png", sep = "" ),
     width = 1024, height = 768 )
dev.control( "enable" )
par( mfrow = c( 1,3 ) )
...
## MEANS
x.mean <- mean_est_divt.post72sp[ind.post.72sp]*100
y.mean <- STnSB.2$mean_divt[ind.post.cal]*100
col.mean <- rep( "black", length( ST.calib.nodes ) )
#col.mean[c(2,3)] <- "red"
plot( x = x.mean, y = y.mean, col = col.mean, pch = 20,
      xlab = expression( 'Posterior mean time '*italic(t)*' (Ma)|72sp - data1' ),
...
text( x = 100, y = 180,
      labels = expression( atop( ''*italic( R^2 )*' = 0.9999' ) ) )
text( x = 100, y = 175,
      labels = expression( atop( ''*italic( w )*' = '*italic( 0.995046 * t )*'' ) ) )

## QUANTILE 2.5 
x.qlow <- quant_est_divt.post72sp[ind.post.72sp,1]*100
y.qlow <- STnSB.2$quant_divt[ind.post.cal,1]*100
plot( x = x.qlow, y = y.qlow, col = col.mean, pch = 20,
      main = subtree, 
      xlab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|72sp - data1' ),
      ylab = expression( 'Quant. 2.5% post. mean time '*italic(t)*' (Ma)|307sp - marsupialia_data2' ),
...
text( x = 100, y = 145,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.000757 * t )*'' ) ) )

## QUANTILE 97.5
x.qup <- quant_est_divt.post72sp[ind.post.72sp,2]*100
y.qup <- STnSB.2$quant_divt[ind.post.cal,2]*100
plot( x = x.qup, y = y.qup, col = col.mean, pch = 20,
      xlab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|72sp - data1' ),
      ylab = expression( 'Quant. 97.5% post. mean time '*italic(t)*' (Ma)|307sp - marsupialia_data2' ),
      xlim = c( min( x.qup )-5, max( x.qup )+ 100 ) )
...
text( x = 110, y = 200,
      labels = expression( atop( ''*italic( R^2 )*' = 0.9999' ) ) )
text( x = 110, y = 195,
      labels = expression( atop( ''*italic( w )*' = '*italic(  1.039754 * t )*'' ) ) )
...
write.table( x = sum.stats.ord, file = paste( "02_SBnST_max0.3_pU0.025_", subtree, "_sumstats.tsv", sep = "" ),
             sep = "\t", quote = FALSE )
