...
for( i in 1:length( check.lineages ) ){
  ...
  # In this case, Marsupialia had been previously filtered and some names 
  # were changed:
  sp     <- gsub( pattern = "marmosa_macrotarsus", replacement = "marmosa_murina_macrotarsus", x = sp )
  sp     <- gsub( pattern = "marmosa_waterhousei", replacement = "marmosa_murina_waterhousei", x = sp )
  sp     <- gsub( pattern = "marmosa_micoureus_paraguayana", replacement = "micoureus_paraguayanus", x = sp )
  sp     <- gsub( pattern = "marmosa_micoureus_demerarae", replacement = "micoureus_demerarae", x = sp )
  sp     <- gsub( pattern = "marmosa_micoureus_regina", replacement = "micoureus_regina", x = sp )
  sp     <- gsub( pattern = "marmosa_micoureus_constantiae", replacement = "micoureus_constantiae", x = sp )
  sp     <- gsub( pattern = "murexia_naso", replacement = "antechinus_naso", x = sp )
  sp     <- gsub( pattern = "murexia_naso", replacement = "phascomurexia_naso", x = sp )
  sp     <- gsub( pattern = "spilocuscus_maculatus", replacement = "phalanger_maculatus", x = sp )
  sp     <- gsub( pattern = "marmosa_simonsi", replacement = "marmosa_robinsoni_simonsi", x = sp )
  sp     <- gsub( pattern = "marmosa_isthmica", replacement = "marmosa_robinsoni_isthmica", x = sp )
