...
ind.path.to.files <- stringr::str_detect( string = path.to.files.raw, pattern = "t.phylip|checked_aln/unfiltered_aln/alignment_nt3cp.phylip" )
...
load.files <- function( path_aln, path_aln_3nt = FALSE, path_partitions, dir_names ){
  ...
  for( i in 1:length( path_aln ) ){
    ...
    ## SAC-200330> OLD CODE. Summarised in lines 181-84
...
## Issues with L. cetartiodactyla and paths
## Try this if the above does not work:
# path.to.files.3nt_v2 <- gsub( pattern ="\\.\\.\\/*", replacement = "", x = path.to.files.3nt )
# wd2 <- gsub( pattern = "01_alignments..*", replacement = "", x = wd )
# path.to.files.3nt_v2 <- paste( wd2, path.to.files.3nt_v2, sep = "" )
# aln.object <- load.files( path_aln = path.to.files, path_aln_3nt = path.to.files.3nt_v2,
#                           dir_names = name.dirs, path_partitions = path.to.partitions )
...
save( aln.object, file = paste( "Rout/Rdata/aln.object.", subt, "_unfiltered.RData", sep = "" ) )
...
store.aln <- function( x, nt3 = FALSE ){
  ...
  for( k in 1:length( tmp.aln ) ){
    ...
    write( cat( "Parsing alignments for ", names( tmp.aln )[k], "...\n" ),
           paste( "Rout/log_concatenation/log_file_concatenate_seqs_", subt, "_unfiltered.txt", sep = "" ),
           append = TRUE, sep = "\n" )
    tmp.aln.k <- tmp.aln[[ k ]]
    tmp.sp.k  <- tmp.sp[[ k ]]
    tmp.nuc.k <- tmp.nuc[[ k ]]
    ...
    if ( ! dir.exists( paste( "00_mammal_alns/", names( tmp.aln )[k], sep = "" ) ) ){
      ...
      dir.create( paste( "00_mammal_alns/", names( tmp.aln )[k], "/unfiltered", sep = "" ) )
    }
    tmp.dirname.k <- paste( "00_mammal_alns/", names( tmp.aln )[k], "/unfiltered/", sep = "" )
    ...
    if( nt3 == FALSE ){
      ...
      cat( "\nThis block should only run if 12CP only or concatenated alignment.phylip!!\n" )
