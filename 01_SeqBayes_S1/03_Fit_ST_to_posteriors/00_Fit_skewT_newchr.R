...
run1    <- read.table( paste( wd_mcmctree,
                              "02_MCMCtree/00_MCMCtree_analyses/00_main_tree_T2/02_MCMCtree_posterior_newchrono/run1/mcmctree_GBM/mcmc.txt",
                              sep = "" ), header = T, sep = "\t" )
run3    <- read.table( paste( wd_mcmctree,
                              "02_MCMCtree/00_MCMCtree_analyses/00_main_tree_T2/02_MCMCtree_posterior_newchrono/run3/mcmctree_GBM/mcmc.txt",
                              sep = "" ), header = T, sep = "\t" )
run4    <- read.table( paste( wd_mcmctree,
                              "02_MCMCtree/00_MCMCtree_analyses/00_main_tree_T2/02_MCMCtree_posterior_newchrono/run4/mcmctree_GBM/mcmc.txt",
                              sep = "" ), header = T, sep = "\t" )
...
divtimes   <- rbind( divtimes1, divtimes3, divtimes4 )
...
if( ! dir.exists( "00_fitST_newchr" ) ){
  dir.create( "00_fitST_newchr" )
}
if( ! dir.exists( "00_fitST_newchr/logs" ) ){
  dir.create( "00_fitST_newchr/logs" )
}
if( ! dir.exists( "00_fitST_newchr/plots" ) ){
  dir.create( "00_fitST_newchr/plots" )
}
if( ! dir.exists( "00_fitST_newchr/Rdata" ) ){
  dir.create( "00_fitST_newchr/Rdata" )
}
if( ! dir.exists( "00_fitST_newchr/Rout" ) ){
  dir.create( "00_fitST_newchr/Rout" )
}
save( divtimes, file = "00_fitST_newchr/Rdata/post.divtimes.RData" )
...
for ( i in 1:length( colnames( divtimes ) ) ){
  ...
  write( paste( "Working with node ", colnames( divtimes )[i], "...\n\n", sep = "" ),
         file = "00_fitST_newchr/logs/log_file_convergence_BFGS.txt", sep = "\n", append = TRUE )
  ...
  png( filename = paste( "00_fitST_newchr/plots/Fit_ST_", colnames( divtimes )[i], ".png", sep = "" ),
  ...
  write( paste( colnames( divtimes )[i], max_x_chain, max_y_chain, sep = "\t" ),
         file = "00_fitST_newchr/logs/log_limaxis.txt", sep = "\n", append = TRUE )
  # 3.2. Plot
...
save( ST.fitted.objects, file = "00_fitST_newchr/Rdata/ST.fitted.objects.RData" )
save( ST.fitted.dists, file = "00_fitST_newchr/Rdata/ST.fitted.dists.RData" )
...
write.table( mat_ST, file = "00_fitST_newchr/Rout/ST.fitted.dists.G2.40.tsv", sep = "\t",
             quote = F )
