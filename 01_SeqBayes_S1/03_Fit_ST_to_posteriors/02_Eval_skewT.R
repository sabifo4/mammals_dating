...
# png( filename = paste( "02_MCMCtree_prior_ST/plots/00_Check_analitST_priorST_posterior.png", sep = "" ),
png( filename = paste( "02_MCMCtree_prior_ST/plots/00_Check_analitST_priorST_posterior_SMALL.png", sep = "" ),
     width = 1024, height = 768 )
# pdf( file = paste( "02_MCMCtree_prior_ST/plots/00_Check_analitST_priorST_posterior_SMALL.pdf", sep = "" ) )
...
png( filename = paste( "02_MCMCtree_prior_ST/plots/00_Check_analitST_priorST.png", sep = "" ),
     width = 1024, height = 768 )
...
for( i in 1:length( colnames( divtimes.prior.ST ) ) ){
  ...
  # 2. Plot fitted ST for each node previously 
  #    computed using the values sampled during the MCMC
  png( filename = paste( "02_MCMCtree_prior_ST/plots/individual_plots/Compare_priorST.vs.analytST_", colnames( divtimes )[i], ".png", sep = "" ),
       width = 1024, height = 768 )
  # 2.1. Find limit axis
  max_x_st            <- round( max( density( rst( n = 1000, dp = tmp_node$dp.complete ) )$x ) + 0.5 )
  max_x_chain_priorST <- round( max( density( divtimes.prior.ST[,i] )$x ) + 0.5 )
